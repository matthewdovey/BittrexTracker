...
final class MarketAPI {
  ...
  final func getBuyLimit(completion: @escaping ((BuyLimitRequest) -> Void)) {
    let url = URL(string: urlBuilder.buildUrl(for: .buyLimit, withParameters: [:]))
  ...
  final func getSellLimit(completion: @escaping ((SellLimitRequest) -> Void)) {
    let url = URL(string: urlBuilder.buildUrl(for: .sellLimit, withParameters: [:]))
    let task = session.dataTask(with: url!) { (data, response, error) in
      if error != nil {
  ...
  final func cancel(completion: @escaping ((CancelRequest) -> Void)) {
    let url = URL(string: urlBuilder.buildUrl(for: .cancel, withParameters: [:]))
    let task = session.dataTask(with: url!) { (data, response, error) in
      if error != nil {
  ...
  final func getOpenOrders(completion: @escaping ((OpenOrdersRequest) -> Void)) {
    let url = URL(string: urlBuilder.buildUrl(for: .openOrders, withParameters: [:]))
    let task = session.dataTask(with: url!) { (data, response, error) in
      if error != nil {
